  ...
  post '/' do
    ...
    when :check_run
      ...
      when :created
        ...
        # We need to be selective, hence the conditional on the app id. We only want to process our _own_ś
        # check runs. That's why we check if the app id is == APP_IDENTIFIER
        logger.debug "created"
        logger.debug @payload['check_run']['app']['id']
        logger.debug APP_IDENTIFIER
        logger.debug @payload['check_run']['app']['id'] == APP_IDENTIFIER
